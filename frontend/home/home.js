...
async function uploadTheme(slot,input){
  ...
  try{
    ...
    if(r.ok){localStorage.setItem('theme_ver',String(Date.now()));toast('已上传');loadThemeUploads();applyUserTheme();}
  }catch(e){toast('上传失败');}
}
...
async function loadThemeUploads(){
  try{
    ...
    ['bg','strip','corner','desk_left','desk_right'].forEach(s=>{
      ...
      if(!prev)return;
      if(slots[s]){prev.style.backgroundImage=`url(${slots[s]}?v=${themeVer()})`;const sp=prev.querySelector('span');if(sp)sp.style.display='none';}
...
function applyBgAdjust(){
  ...
  const blur=getBgBlur(), feather=getBgFeather(), px=getBgPosX(), py=getBgPosY(), zoom=(getBgZoom()||100)/100;
  bg.style.backgroundSize='cover';
  bg.style.backgroundPosition=px+'% '+py+'%';                 // 裁切位置：图的哪个部分对齐到屏幕
  const scale=Math.max(1,zoom)+(blur>0?0.08:0);              // 缩放（放大裁得更紧）；模糊时再多放一点藏掉边缘透明晕
  bg.style.transform=(scale!==1)?`scale(${scale})`:'';
  bg.style.transformOrigin=px+'% '+py+'%';                    // 以裁切焦点为中心缩放
...
async function applyUserTheme(){
  try{
    ...
    if(slots.bg){
      ...
      bg.style.backgroundImage=`url(${slots.bg}?v=${themeVer()})`;bg.classList.remove('tiled');
      document.body.classList.add('has-bg');
    }else{
      // 关键：删掉背景后要真正清掉（旧版没有这个分支 → 删了背景还留着，导致“去不掉背景”）
      bg.style.backgroundImage='';document.body.classList.remove('has-bg');
    }
