...
def _lib_conn(path, row=False, timeout=60.0):
    """打开 library.db 的写连接，统一配 WAL + busy_timeout。
    关键：原来各任务用裸 sqlite3.connect()，busy_timeout 默认为 0，
    两个后台任务同时写时第二个会瞬间撞 'database is locked' 直接报错——这就是“管理任务没法并发”的根。
    配上 WAL（写不阻塞读）+ 大 busy_timeout（写写冲突时排队等待而非立刻失败），
    互不冲突的任务就能并发跑、偶发争锁也会自动等待重试。"""
...
def task_dedup_scan(task: Task, root: Path):
    ...
    # 第四层：佚名/空作者跨状态去重——佚名或空作者的书 vs 有作者的同名书，前三层按作者 key 分组时照不了面，这里补上。
    # 只处理前三层完全没碰过的书（避免重叠、避免数据迁移链断裂），且仅在"同一规范化书名 + 恰好 1 个真实作者
    # + 至少 1 本佚名/空作者"时合并：真实作者锚定身份，佚名那些多半就是丢了作者的同一本。全佚名(无真实作者锚)
    # 的同名书容易撞名、稳妥不在这里合并（佚名互重仍由第二层按字数接近处理）。≥2 个真实作者也跳过。
    # keep/remove 先随便放，交给 _refine 按文件大小/章节重排；且有"不确定就并存"兜底，撞名也不会误删。
    task.set_progress(78, '第四层：佚名/空作者跨状态去重...')
    grouped_ids = set()
    for _ly in ('hash', 'title', 'stub'):
        for _g in groups[_ly]:
            grouped_ids.add(_g['keep']['id'])
            grouped_ids.update(b['id'] for b in _g['remove'])
    ANON_SET = {'佚名', '匿名', '未知', '无名', '未知作者'}
    title_pool = defaultdict(list)
    for r in conn.execute('''SELECT id, title, author, file_path, file_size, word_count,
                                    quality_score, is_favorite, is_read, chapter_count, extra_count
                             FROM books'''):
        if r['id'] in grouped_ids:
            continue
        nt = _norm_title(r['title'] or '')
        if not nt:
            continue
        title_pool[nt].append(dict(r))
    cross_added = 0
    for nt, bks in title_pool.items():
        if len(bks) < 2 or len(bks) > 60:   # >60 多半是规范化撞名的噪声组，跳过留人工
            continue
        real_authors = set()
        anon_n = 0
        for b in bks:
            a = (b['author'] or '').strip()
            if a and a not in ANON_SET:
                real_authors.add(a)
            else:
                anon_n += 1
        if len(real_authors) != 1 or anon_n == 0:
            continue
        groups['title'].append({'keep': bks[0], 'remove': bks[1:]})
        for b in bks[1:]:
            removed_ids.add(b['id'])
        cross_added += 1
    if cross_added:
        task.log(f'第四层跨状态：又找出 {cross_added} 组佚名漏掉作者、与有作者版同名的重复')
    ...
    # refine 之后重新统计：不确定的版本已挪进 keep_extra 一并保留，按真正会清理的 remove 重算
    n_hash = sum(len(g['remove']) for g in groups['hash'])
    n_title = sum(len(g['remove']) for g in groups['title'])
    n_stub = sum(len(g['remove']) for g in groups['stub'])
    total_remove = n_hash + n_title + n_stub
    ...
    for _layer in ('hash', 'title', 'stub'):
        for _g in groups[_layer]:
            _g['gid'] = _gid; _gid += 1
    preview = _build_preview(groups, root=root, limit=3000)
...
def _eff_chapters(b):
    # 取这本书的有效章节/番外（优先库里存的 chapter_count，其次现场数的 _ch）；防误数
...
def _dup_comp(b):
    ch, ex = _eff_chapters(b)
    return {'sz': b.get('file_size') or 0, 'ch': ch,
            'wc': b.get('word_count') or 0, 'ex': ex,
            'fav': b.get('is_favorite') or 0}


def _dup_keyval(b):
    # 选"保留哪版"的排序键：收藏 > 非空 > 文件大小 > 章节 > 字数 > 番外。
    # 关键改动：以文件大小为主信号（最实在的内容量），不再让可能过时/灌水的字数当第一位——
    # 之前 0.0MB 的残档却标着 1.7万字，靠字数就把它误选成"保留"了。近乎空的文件直接垫底。
    m = _dup_comp(b)
    not_empty = 0 if (m['sz'] < 8 * 1024 or m['wc'] <= 0) else 1
    return (m['fav'], not_empty, m['sz'], m['ch'], m['wc'], m['ex'])


def _split_dups(allb, keep):
    """给定一组重复书 + 选定的保留版，返回 (一并保留的不确定版, 要清理的版)。
    只清掉两类：① 近乎空 / 远小于保留版的碎片（其字数不可信）；② 在 大小+章节+字数 三项上
    都不优于保留版（严格更差）的。其余只要在某一维更优 = 拿不准谁更全 → 一并保留，免得误删
    了其实更全的版本（这就是"不确定的重复多留几个版本"）。"""
    km = _dup_comp(keep)
    keep_extra, remove = [], []
    for v in allb:
        if v is keep:
            continue
        vm = _dup_comp(v)
        if vm['sz'] < 30 * 1024 or (km['sz'] > 0 and vm['sz'] < km['sz'] * 0.10):
            remove.append(v)
            continue
        if km['sz'] >= vm['sz'] and km['ch'] >= vm['ch'] and km['wc'] >= vm['wc']:
            remove.append(v)
        else:
            keep_extra.append(v)
    return keep_extra, remove
...
def _refine_keep_by_chapters(groups, root, limit_per=400):
    ...
    for layer in ('hash', 'title', 'stub'):
        ...
        for g in groups[layer]:
            allb = [g['keep']] + g.get('keep_extra', []) + g['remove']
            ...
            for b in allb:
                ...
                try:
                    ...
                    pass
                b['_ch'], b['_ex'] = ch, ex
            # 选最全的那版保留（文件大小为主，见 _dup_keyval）；其余拿不准的一并保留、只清严格更差的
            allb.sort(key=_dup_keyval, reverse=True)
            keep = allb[0]
            keep_extra, remove = _split_dups(allb, keep)
            g['keep'] = keep
            g['keep_extra'] = keep_extra
            g['remove'] = remove
            # 把有效章节回填，供预览展示
            for b in allb:
                ch, ex = _eff_chapters(b)
...
def _build_preview(groups, root=None, limit=200):
    ...
    for layer, label in [('hash', 'hash相同'), ('title', '书名相似'), ('stub', '空壳残章')]:
        for g in groups[layer][:limit//3]:
            all_books = [g['keep']] + g.get('keep_extra', []) + g['remove']
            ...
            preview.append({
                ...
                'keep_extra': [book_info(b) for b in g.get('keep_extra', [])],
            ...
            for g in groups[layer]:
                ...
                if gid in ov:
                    ...
                    if chosen and chosen is not g['keep']:
                        g['keep'] = chosen
                        g['keep_extra'], g['remove'] = _split_dups(allb, chosen)
                        swapped += 1
        if swapped:
...
def estimate_classify_cost(root: Path, scope: str, scope_value: str = None) -> dict:
    """预估打标签的范围和成本"""
    conn = _lib_conn(root / 'library.db')
    conn.row_factory = sqlite3.Row
...
def task_recover_authors(task: Task, root: Path, apply: bool = False):
    ...
    # 从佚名书名里按词频推断作者（同一作者名作为首/尾段反复出现在多本不同书里），补进已知作者集，
    # 这样"书名_作者"这种没有 作者/by 标记、作者又还不在库里的，也能被匹配上。仍走预览，过目后再应用。
    anon_titles = [r['title'] or '' for r in rows]
    anon_titles += [r['raw_title'] for r in rows if r['raw_title']]
    inferred = ra.infer_authors_by_frequency(anon_titles, known)
    if inferred:
        known = known | inferred
        task.log(f'按书名词频推断出 {len(inferred)} 个疑似作者，一并用于匹配（如 {("、".join(sorted(inferred)[:8]))} …）')
...
def task_detect_finished(task: Task, root: Path, resume: bool = True):
    ...
    if batch_id:
        try:
            ...
            pass
    # 全库累计统计：续跑时本次可能只扫了几本，但用户想看的是全库到底多少完结
    # （之前只报"本次扫的"，断点续跑后看着像"只有 2 本完结"，其实那是这次新扫的 2 本）
    cum_fin = cum_ong = cum_unk = 0
    try:
        cum_fin = conn.execute('SELECT COUNT(*) c FROM books WHERE is_finished=1').fetchone()['c']
        cum_ong = conn.execute('SELECT COUNT(*) c FROM books WHERE is_finished=0').fetchone()['c']
        cum_unk = conn.execute('SELECT COUNT(*) c FROM books WHERE is_finished IS NULL').fetchone()['c']
    except Exception:
        pass
    conn.close()
