...
def api_shelf(offset: int = 0, limit: int = 0, q: str = ''):
    ...
    try:
        ...
        note_counts = {}
        book_genres = {}
        prog = {}   # book_id -> (percentage, position) 真实阅读进度（取每本最新一条）
        # 主查询：只取书架需要的字段
        books = []
        has_new_cols = True
        try:
        ...
        if limit and limit > 0 and len(ids) <= 2000:
            for chunk in _chunks(ids):
                ...
                try:
                    ...
                    pass
                try:
                    for r in c.execute(f'SELECT book_id, percentage, position FROM reading_progress WHERE id IN (SELECT MAX(id) FROM reading_progress WHERE book_id IN ({ph}) GROUP BY book_id)', chunk):
                        prog[r['book_id']] = (r['percentage'] or 0, r['position'] or 0)
                except Exception:
                    pass
        else:
            try:
            ...
            try:
                ...
                pass
            try:
                for r in c.execute('SELECT book_id, percentage, position FROM reading_progress WHERE id IN (SELECT MAX(id) FROM reading_progress GROUP BY book_id)'):
                    prog[r['book_id']] = (r['percentage'] or 0, r['position'] or 0)
            except Exception:
                pass
        def _progpct(bid, wc):
            # 真实进度：优先用存的百分比；没有就用 位置/字数 现算；都没有则 0（不再瞎兜底成 30）
            pct, pos = prog.get(bid, (0, 0))
            if pct and pct > 0:
                return max(0, min(100, round(pct)))
            if wc and pos:
                return max(0, min(100, round(pos / wc * 100)))
            return 0
        for r in rows:
            books.append({
                ...
                'pc': _progpct(r['id'], r['word_count'] or 0),
...
async def api_ai_chat(request: Request):
    ...
    try:
        ...
        if not result.get('error'):
            reply_id = add_message(dbp, session_id, 'assistant', result.get('reply', ''))
            result['message_id'] = reply_id   # 供前端「长按收藏」这条回复
        result['session_id'] = session_id
    except Exception:
        pass
    return result

@app.get('/api/study/limit')
async def api_study_limit():
    """返回当前配置模型下、单次可读的正文字数上限（前端据此提示，不再写死）。"""
    ai_cfg = load_config().get('ai', {})
    from study_agent import context_char_cap
    return {'cap': context_char_cap(ai_cfg), 'model': ai_cfg.get('model', '')}


@app.post('/api/study/outline')
async def api_study_outline(request: Request):
    """从选定范围[start,end]抽取结构化知识点，并把每个点锚定到正文字符位置（供前端跳转）。"""
    body = await request.json()
    book_id = body.get('book_id')
    try:
        start = int(body.get('start', 0)); end = int(body.get('end', 0))
    except Exception:
        raise HTTPException(400, 'bad range')
    if not book_id or end <= start:
        raise HTTPException(400, 'bad range')
    lib = get_lib()
    r = lib.conn.execute('SELECT file_path, file_ext FROM books WHERE id=?', (book_id,)).fetchone()
    if not r:
    ...
    full = lib.root / r['file_path']
    ext = (r['file_ext'] or '').lower().lstrip('.')
    book_text = read_txt_cached(full) if (ext in ('txt', 'text') and full.exists()) else ''
    if not book_text:
        return {'error': '这本书暂不支持梳理（仅支持 txt）', 'points': []}
    start = max(0, start); end = min(end, len(book_text))
    passage = book_text[start:end]
    if len(passage.strip()) < 10:
        return {'error': '选中的范围太短', 'points': []}
    ai_cfg = load_config().get('ai', {})
    if not ai_cfg.get('api_key'):
        return {'error': 'no_api_key', 'points': []}
    from study_agent import make_outline
    data = make_outline(passage, ai_cfg, str(body.get('mode', 'points')))
    if data.get('error'):
        return {'error': data['error'], 'points': []}
    # 用 quote 在选段里定位，换算成全书字符偏移
    def _norm(s):
    ...
    for p in (data.get('points') or []):
        ...
        if q:
            ...
            if idx >= 0:
                pos = start + idx
        points.append({'title': p.get('title', ''), 'detail': p.get('detail', ''),
                       'quote': q, 'position': pos})
    return {'points': points, 'start': start, 'end': end}


@app.post('/api/study/quiz')
async def api_study_quiz(request: Request):
    """从选定范围[start,end]出单选题，并把每题考点锚定到正文位置。"""
    body = await request.json()
    book_id = body.get('book_id')
    try:
        start = int(body.get('start', 0)); end = int(body.get('end', 0)); n = int(body.get('n', 4))
    except Exception:
        raise HTTPException(400, 'bad range')
    if not book_id or end <= start:
        raise HTTPException(400, 'bad range')
    ...
    book_text = read_txt_cached(full) if (ext in ('txt', 'text') and full.exists()) else ''
    if not book_text:
        return {'error': '这本书暂不支持出题（仅支持 txt）', 'questions': []}
    start = max(0, start); end = min(end, len(book_text))
    passage = book_text[start:end]
    if len(passage.strip()) < 20:
        return {'error': '选中的范围太短，出不了题', 'questions': []}
    ai_cfg = load_config().get('ai', {})
    if not ai_cfg.get('api_key'):
        return {'error': 'no_api_key', 'questions': []}
    from study_agent import make_quiz
    data = make_quiz(passage, ai_cfg, n)
    if data.get('error'):
        return {'error': data['error'], 'questions': []}
    ...
    for q in (data.get('questions') or []):
        ...
        if quote:
            ...
            if idx < 0:
                qn = _norm(quote); jn = passage_norm.find(qn)
                if jn >= 0:
                    cnt = 0
                    for i, ch in enumerate(passage):
                        if not ch.isspace():
        ...
        q['position'] = pos
        questions.append(q)
    return {'questions': questions, 'start': start, 'end': end}
